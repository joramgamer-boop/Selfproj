...
- [ ] A Stop can be moved on an open Position, recorded as a timestamped
      `StopMoved` event
- [ ] Tightening (moving the Stop toward entry) is allowed without a block
- [ ] Rule: widening (moving the Stop away from entry) is blocked, overridable,
      and records a Violation
- [ ] **1R remains fixed at the Plan's original Stop** regardless of any number of
      moves
- [ ] The current Stop and the original Stop are both visible on the open Position
- [ ] Core tests confirm R-multiple on a Trade closed after a tightened Stop is
